...
description: >-
  Offers the user an informed choice about response depth before
  answering. Use when the user explicitly wants to control response
  length, depth, or token budget.
  TRIGGER on: "token budget", "token usage", "token limit", "response
  length", "answer depth", "short version", "brief answer", "detailed
  answer", "exhaustive answer", "cuántos tokens", "ahorrar tokens",
  "dame la versión corta", or clear variants asking to control answer
  size. DO NOT TRIGGER when a level is already set this session, the
  answer is one word, or "token" means auth/payment tokens.
metadata:
  origin: community
---
